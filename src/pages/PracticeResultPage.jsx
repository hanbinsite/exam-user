...
export default function PracticeResultPage() {
  ...
  const raw = localStorage.getItem('practice-result');
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-2">没有练习记录</p>
          <p className="text-gray-400 text-sm mb-4">请先完成一次练习</p>
          <button
            onClick={() => navigate('/')}
  ...
  const unanswered = questions.filter(q => answers[String(q.id)] === undefined).length;
  const accuracy = (total - unanswered) > 0 ? Math.round((correctCount / (total - unanswered)) * 100) : 0;

  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
      <div className="bg-gradient-to-r from-emerald-600 via-teal-500 to-cyan-500 text-white py-8 px-4 shadow-lg">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-white/80">来看看你的表现吧</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-gray-100">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-4 text-center border border-amber-100">
              ...
              <div className="text-sm text-amber-600 mt-1 font-medium">正确率</div>
            </div>
          </div>
          {unanswered > 0 && (
            <p className="text-center text-gray-400 text-sm mt-4">（未作答 {unanswered} 题）</p>
          )}
        </div>
