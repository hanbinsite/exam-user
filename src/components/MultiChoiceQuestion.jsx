...
export default function MultiChoiceQuestion({ question, index }) {
  ...
  const isCorrect = showFeedback && isAnswered && selectedArr.length === answerArr.length && selectedArr.every(k => answerArr.includes(k));
  ...
  const handleToggle = (optionKey) => {
    ...
    if (mode !== 'exam' && isAnswered && showFeedback) return;
    let next;
    if (isSelected(optionKey)) {
        ...
        {question.options.map((option) => (
          ...
          >
            <span className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-semibold text-sm transition-all ${
              showFeedback && isAnswered && isCorrectOption(option.key)
                ? 'bg-green-500 text-white'
                : showFeedback && isAnswered && isSelected(option.key) && !isCorrectOption(option.key)
                  ? 'bg-red-500 text-white'
                  : !showFeedback && isAnswered && isSelected(option.key)
