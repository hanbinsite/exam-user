...
export default function MultiChoiceQuestion({ question, index }) {
  ...
  const answerArr = normalizeMultiAnswer(question.answer);
        ...
        <div className={`mt-3 lg:mt-5 ml-0 sm:ml-14 p-3 lg:p-4 rounded-lg lg:rounded-xl flex items-center gap-2 lg:gap-3 ${
          ...
          {isCorrect ? (
            ...
            <>
              ...
              <span className="font-medium text-sm lg:text-base">回答错误，正确答案是 <span className="bg-white px-2 py-1 rounded font-bold">{normalizeMultiAnswer(question.answer).join('、')}</span></span>
            </>
          )}
        </div>
      )}
